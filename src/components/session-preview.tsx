...
// Number of rendered-buffer entries to keep cached. Eight is enough to
// cover a typical "rummage through recent sessions" flow without growing
...
// the same on a cache hit. Eviction trims from the head (oldest).
//
// Key carries every input that changes the visible output:
//   sessionId — content identity
//   width     — wrap boundaries differ per pane width
//   lang      — role headers and tool labels are localized
//   emoji     — toggling emoji rewrites every message header
// `displayMode` doesn't need to be in the key — toggling it changes the
// `messages` prop, which re-fires the render effect and overwrites the
// cache entry with the new buffer.
const bufferCache = new Map<string, ConversationBuffer>();

function bufferCacheKey(sessionId: string | null, width: number, lang: string, emoji: boolean): string {
  return `${sessionId ?? ""}|${width}|${lang}|${emoji ? "e" : "p"}`;
}
